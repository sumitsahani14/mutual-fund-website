  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
        ...
        <span className="shrink-0 rounded-lg bg-muted px-3 py-1 text-sm font-600 text-royal" style={{ fontWeight: 600 }}>
          {unit === '₹' ? fmt(value) : `${value}${unit || ''}`}
        </span>
      </div>
      <input type="range" min={min} max={max} step={step} value={value}
        onChange={(e) => set(Number(e.target.value))}
        className="range-input mt-4 h-2 w-full cursor-pointer appearance-none rounded-full bg-muted accent-[hsl(var(--royal))]"
  ...
  return (
    <div className="rounded-2xl border border-border bg-white p-5 shadow-sm sm:p-6">
      <div className="space-y-3">
        {rows.map((r) => (
          <div key={r.label} className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 border-b border-border/60 pb-3 last:border-0 last:pb-0">
      ...
      {highlight && (
        <div className="mt-5 rounded-xl navy-gradient p-4 text-center text-white sm:p-5">
          <p className="text-xs uppercase tracking-wider text-white/60">{highlight.label}</p>
          <p className="mt-1 break-words font-display text-2xl font-700 sm:text-3xl" style={{ fontWeight: 700 }}>{highlight.value}</p>
        </div>
      )}
    </div>
  );
...
function Shell({ controls, result }) {
  return (
    <div className="grid gap-6 md:grid-cols-2 md:gap-8">
      <div className="space-y-6 rounded-2xl border border-border bg-white p-5 shadow-sm sm:space-y-7 sm:p-6">{controls}</div>
      <div>{result}</div>
    </div>
  );
}
